...
class GazeboEnv(Node):
    ...
    def step(self, action):
        ...
        if box_detected:
            self.get_logger().info("Box detected, adjusting actions to move towards it.")
            # Adjust actions to move towards the box
        ...
        reward = self.get_reward(target, collision, action, min_laser, self.box_contact, timeout, box_detected)

        # Debug logging for action and reward
        self.get_logger().info(f"Step result - reward: {reward}, done: {done}, box_detected: {box_detected}")
    ...
    @staticmethod
    def get_reward(target, collision, action, min_laser, box_contact, timeout, box_detected):
        if target:
            return 1000.0
        elif collision:
            return 20.0  # Positive reward for touching the box
        elif timeout:
            return -500.0  # Negative reward for timeout
        elif box_contact:
            return 50.0  # Additional reward for touching the box
        elif box_detected:
            return 10.0
        else:
            # Provide balanced incentives for forward movement and turning
            forward_reward = 1.0 - abs(action[0])  # Reward for moving forward
            turn_penalty = 0.5 * abs(action[1])  # Penalize excessive turning
            return forward_reward - turn_penalty - (1 - min_laser if min_laser < 1 else 0.0) / 2
    ...
    def observe_collision(laser_data):
        ...
        return False, collision, min_laser
